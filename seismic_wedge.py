...
depth_rc_model, earth, imp, time_vec, time_rc_model = make_wedge(width=length,depth=depth,l1t=depth/4,theta=theta, dt=dt, noise=False, noise_var=0.1)
depth_rc_model_n, earth, imp, time_vec, time_rc_model_n = make_wedge(width=length,depth=depth,l1t=depth/4,theta=theta, dt=dt, noise=True, noise_var=0.1)
...
w = bruges.filters.ricker(duration=0.100, dt=dt, f=40)
...
plt.xlabel('Distance (m)')
plt.ylim(0,np.max(np.max(apparent)))
plt.xlim(0,length)
plt.legend()
#plt.savefig('ch2_synthetic_amplitude_curve2.jpg', dpi=300)
plt.show()
...
plt.legend()
#plt.savefig('ch2_synthetic_wedge_seismic2.jpg', dpi=300)
plt.show()
...
#plt.savefig('ch2_synthetic_wedge_setup2.jpg', dpi=300)
plt.show()
