...
## Make CSV files for exporting for Craig 
greatest_southland_detrended.drop(['line', 'amplitude', 'depth','decompacted_profile_km', 'fold_profile_detrended', 'y_utm'], axis=1, inplace=True)
least_southland_detrended.drop(['line', 'amplitude', 'depth','decompacted_profile_km', 'fold_profile_detrended', 'y_utm'], axis=1, inplace=True)
greatest_sill_amp_from_time.drop(['line', 'sill_amp_time'], axis=1, inplace=True)
least_sill_amp_from_time.drop(['line', 'sill_amp_time'], axis=1, inplace=True)

result = pd.merge(greatest_southland_detrended, least_southland_detrended, how='outer', on=['x_utm','trace'], suffixes=('_greatest', '_least'))
result2 = pd.merge(greatest_sill_amp_from_time, least_sill_amp_from_time, how='outer', on=['trace','x_utm'], suffixes=('_greatest','_least'))
final_result = pd.merge(result, result2, how='outer', on=['x_utm','trace'])
final_result.sill_amp_meters_greatest /= 1000
final_result.sill_amp_meters_least /= 1000

final_result.to_csv("Resolution_decompaction.csv")
